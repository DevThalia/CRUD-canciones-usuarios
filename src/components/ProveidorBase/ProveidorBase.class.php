...
class ProveidorBase
{
    ...
    {
        if (!esEnter($idProveidor)) {
            die('[ERROR a Proveidor.class] $idProveidor ha de ser un int');
        } //validació de tipus
        if (!($db instanceof Db)) {
            die("[ERROR a Proveidor.class] l'objecte Db no s'ha passat al contructor o no es correcte");
        } //validació de tipus
        $this->db = $db; //desem la connexió a la bbdd
        //carreguem les dades de l'entrada o inicialitzem

        if ($idProveidor != 0) {
            $this->carregaDades($idProveidor);
        } else {
            $this->reset();
        }
    }
    ...
    public function set($camp, $valor)
    {
        ...
        switch ($camp) {
            ...
            case "idProveidor":
                $tipoValor = "enter";
                if (esTipo($tipoValor, $valor)) {
                    $this->$camp = $valor;
                    return TRUE;
                } else {
                    $this->msgError = "El valor " . $valor . " del camp " . $camp . " no és $tipoValor";
                }
                break;
                //la resta de camps
            case "nomProveidor":
                $tipoValor = "string";
                if (esTipo($tipoValor, $valor)) {
                    $this->resultatSet = TRUE;
                } else {
                    $this->msgError = "El valor " . $valor . " del" . $camp . " no és $tipoValor";
                }
                break;
            case "emailProveidor":
                $tipoValor = "string";
            ...
            case "nomFacturacioProveidor":
                ...
                break;
            case "direccioFacturacioProveidor":
                $tipoValor = "string";
                if (esTipo($tipoValor, $valor)) {
                    $this->resultatSet = TRUE;
                } else {
            ...
            case "cpFacturacioProveidor":
                ...
                break;
            case "poblacioFacturacioProveidor":
                $tipoValor = "string";
                if (esTipo($tipoValor, $valor)) {
                    $this->resultatSet = TRUE;
                } else {
                ...
                break;
            case "provinciaFacturacioProveidor":
                $tipoValor = "string";
                if (esTipo($tipoValor, $valor)) {
                    $this->resultatSet = TRUE;
                } else {
                ...
                break;
            case "codiPaisFacturacioProveidor":
                $tipoValor = "string";
                if (esTipo($tipoValor, $valor)) {
                    $this->resultatSet = TRUE;
                } else {
            ...
            case "tokenProveidor":
                ...
                break;
            case "correuFacturacioProveidor":
                $tipoValor = "string";
                if (esTipo($tipoValor, $valor)) {
                    $this->resultatSet = TRUE;
                } else {
            ...
            case "idEmpresa":
                ...
                if (esTipo($tipoValor, $valor)) {
                    ...
                    $this->msgError = "El camp " . $camp . " no és " . $tipoValor;
                }
                break;
            case "flagActiuProveidor":
                $tipoValor = "enter";
                if (esTipo($tipoValor, $valor)) {
                    $this->resultatSet = TRUE;
                } else {
                ...
                break;
            case "longitudComptaComptableProveidor":
                $tipoValor = "enter";
                if (esTipo($tipoValor, $valor)) {
                    $this->resultatSet = TRUE;
                } else {
            ...
            default:
                $this->msgError = "No hi ha el camp";
        }
    ...
    public function get_errorMysql()
    {
    ...
    public function get_idProveidor()
    {
        return (int)$this->idProveidor;
    }
    public function get_tipusError()
    {
        return $this->tipusError;
    }
    ...
    public function getIds($filtre = 0, $ordre = 0, $limit = 0, $group = 0)
    {
        ...
        if ($group) {
            $this->sql_query .= " GROUP BY " . $group;
        }
        if ($ordre) {
            $this->sql_query .= " ORDER BY " . $ordre;
        }
        if ($limit) {
    ...
    {
        if ($idProveidor) {
            if (!$this->set('idProveidor', $idProveidor)) return FALSE;
        }
        if ($this->idProveidor) {
    ...
    function insereix()
    {
        ...
        if ($this->nomProveidor) {
            ...
            if ($this->idProveidor = $this->db->get_ultima_mysql_insert_id()) {
                return TRUE;
            } else {
                $this->msgError = "no s'ha pogut afegir el proveidor";
                $this->errorMysql = $this->db->error();
            }
        } else {
            $this->msgError = "falta el camp de nom proveidor";
        }
        return false;
    }
    ...
    function elimina()
    {
        if ($this->idProveidor) {
            ...
            if ($this->db->query($this->sql_query)) {
                ...
                $this->errorMysql = $this->db->error();
                $this->msgError = "no se ha podido eliminar el proveedor";
            }
        } else {
            $this->msgError = "falta identificador";
        }
        return FALSE;
    }
